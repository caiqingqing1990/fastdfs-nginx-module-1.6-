...
#define FDFS_MAX_STORAGES_IN_SAME_GROUP 5
...
static bool allow_outer_group_visit = false;
...
static int storage_same_group_ip_sleep_interval = 1 * 60;   // sleep 5 minitus

static int fdfs_get_params_from_tracker();
static int fdfs_format_http_datetime(time_t t, char *buff, const int buff_size);
static void same_group_ip_list_get_thread(void);
static char *same_group_ip_arrays[FDFS_MAX_STORAGES_IN_SAME_GROUP];
static int fdfs_get_storage_group_storage_list(bool * volatile continue_flag, const bool client_bind_addr, const char *bind_addr,FDFSStorageInfo *storage_infos);
...
int fdfs_mod_init()
{
	...
	allow_outer_group_visit = iniGetBoolValue(NULL, "allow_outer_group_visit", \
						&iniContext, false);
	...
	logInfo("fastdfs apache / nginx module v1.15, " \
		...
		response_mode == FDFS_MOD_REPONSE_MODE_PROXY ? \
			"proxy" : "redirect", \
		g_fdfs_base_path, url_have_group_name,allow_outer_group_visit, buff, \
	...
	// TODO start a thread to get lists in the same group

    pthread_t same_group_ip_list_get_thread_id;
    int ret;  
    ret = pthread_create(&same_group_ip_list_get_thread_id,NULL,(void *)same_group_ip_list_get_thread,NULL);

     if(ret!=0)    //  线程创建成功返回0  
    {  

    logError("[fastdfs-fb2]file: "__FILE__", line: %d, " \
				"create same_group_ip_list_get_thread error,create error code %d",__LINE__, \
				ret);

    }else
    {
      logInfo("[fastdfs-fb2]file: "__FILE__", line: %d, " \
				"create same_group_ip_list_get_thread success.",__LINE__);
    }  
...
#define HTTPD_MAX_PARAMS   32
	...
	else
	{
		...
		else if (pContext->proxy_handler != NULL)
		{
			...
			if (allow_outer_group_visit)
			{
					return pContext->proxy_handler(pContext->arg, \
					file_info.source_ip_addr);
			}

			// check source_ip is in the same group list
			// TODO
			int a ;
			for (a = 0 ;a < FDFS_MAX_STORAGES_IN_SAME_GROUP;a++){

				if (strcmp(file_info.source_ip_addr,same_group_ip_arrays[a]) == 0){

								return pContext->proxy_handler(pContext->arg, \
					file_info.source_ip_addr);
				}

				logWarning("[fastdfs-fb2]file: "__FILE__", group id %d,group ip %s",a,same_group_ip_arrays[a]);
			}

			logWarning("[fastdfs-fb2]file: "__FILE__", line: %d, " \
				"source ip %s is not in the same group with the current storage,not allow to visit, " \
				"uri: %s", __LINE__, file_info.source_ip_addr,uri);
...
// start a thread to get ips
void same_group_ip_list_get_thread(void)
{

	while(true){

		logInfo("[fastdfs-fb2]load same group storages from tracker start.");

        //*same_group_ip_arrays[FDFS_MAX_STORAGES_IN_SAME_GROUP] = {"10.10.30.99","10.10.30.98","","",""};
        // same_group_ip_arrays[0] = "10.10.30.99";
        // same_group_ip_arrays[1] = "10.10.31.188";
        // same_group_ip_arrays[2] = "10.10.30.188";

        // connect tracker and get storage ip

	    FDFSStorageInfo storage_infos[FDFS_MAX_STORAGES_IN_SAME_GROUP];
	    int result;
	    bool continue_flag;

	    continue_flag = false;

	  if ((result=fdfs_get_storage_group_storage_list(&continue_flag, false, NULL,storage_infos)) != 0)
        {

           logInfo("[fastdfs-fb2]load same group storages ip from tracker failed,result %d.",result);

        }else{

        	logInfo("[fastdfs-fb2]load same group storages ip from tracker success.");
        }

        int s ;

        for (s = 0;s<FDFS_MAX_STORAGES_IN_SAME_GROUP;s++)
        {
        	same_group_ip_arrays[s] = storage_infos[s].ip_addr;
        }

        logInfo("[fastdfs-fb2]get same group storage ip from tracker,ip list %s , %s, %s, %s, %s .",\
        	same_group_ip_arrays[0],same_group_ip_arrays[1],same_group_ip_arrays[2],\
        	same_group_ip_arrays[3],same_group_ip_arrays[4]);

		logInfo("[fastdfs-fb2]load same group storages ip from tracker end.");

		sleep(storage_same_group_ip_sleep_interval);

	}

}

int fdfs_get_storage_group_storage_list(bool * volatile continue_flag, \
		const bool client_bind_addr, const char *bind_addr,FDFSStorageInfo *storage_infos)
{

    int storage_count;


    TrackerServerGroup *pTrackerGroup = &g_tracker_group;
			...
			pGlobalServer++)
	{
        ...
        result = tracker_list_servers(pTServer, \
		group_store_paths[i].group_name, NULL, \
		storage_infos, FDFS_MAX_STORAGES_IN_SAME_GROUP, \
		&storage_count);
